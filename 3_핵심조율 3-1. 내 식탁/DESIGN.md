...
## Screen-specific notes (3_핵심조율 3-1. 내 식탁, 구 "홈-카드목록")

- **2026-08-16 개명**: 이 화면은 더 이상 하단 탭의 "홈"이 아니다. 진짜 "홈"([3_핵심조율 3-0. 홈](../3-0.%20홈/DESIGN.md))이 가족 모두가 보는 중립적 요약이 되었고, 이 화면은 로그인한 한 사람의 역할(호스트/게스트) 기반 카드·응답·확정 액션을 보여주는 "한 단계 다음" 화면이다. "가족이 먹고 싶어해요"는 공유 공간인 3-0으로 옮겼다.
